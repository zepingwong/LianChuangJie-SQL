...
SELECT T.Brand,
       T.Modle,
       ISNULL(PurchaseOrder.PurchaseFrequency, 0)                        AS PurchaseFrequency, /*近一年采购频次总数*/
       ROW_NUMBER() OVER (ORDER BY PurchaseOrder.PurchaseFrequency DESC) as PurchaseFrequencyRank /*近一年采购频次排名*/
FROM (
         /*近一年询报价业务所涉及的品牌、型号*/
