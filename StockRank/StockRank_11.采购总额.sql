...
SELECT T.Brand,
       T.Modle,
       ISNULL(PurchaseOrder.SumPurchaseMoney, 0)                        AS SumPurchaseMoney, /*近一年采购总额*/
       ROW_NUMBER() OVER (ORDER BY PurchaseOrder.SumPurchaseMoney DESC) as SumPurchaseMoneyRank /*近一年采购总额排名*/
FROM (
         /*近一年询报价业务所涉及的品牌、型号*/
